...
-- 11. Profiles RLS & Auto-insertion (Prevents Foreign Key Violations)
...
select 
  id, 
  email, 
  coalesce(raw_user_meta_data->>'full_name', raw_user_meta_data->>'name', split_part(email, '@', 1), 'HackFlow Member'),
...
-- Ensure auto-creation trigger on new user signup
...
-- 12. Friends, Squads, and Event Participants Layer
...
-- Backfill existing team_members to event_participants
do $$
begin
...
returns boolean language sql security definer set search_path = public stable as $$
  select exists (select 1 from public.event_participants where event_id = p_event_id and user_id = p_user_id);
$$;

create or replace function public.is_event_team_member(p_event_id uuid, p_user_id uuid)
returns boolean language sql security definer set search_path = public stable as $$
  select exists (select 1 from public.event_participants where event_id = p_event_id and user_id = p_user_id)
      or exists (select 1 from public.team_members where event_id = p_event_id and user_id = p_user_id);
$$;

-- RLS for Friendships
alter table public.friendships enable row level security;
drop policy if exists "friendships_select" on public.friendships;
create policy "friendships_select" on public.friendships for select using (auth.uid() = sender_id or auth.uid() = receiver_id or receiver_email = auth.jwt()->>'email');
drop policy if exists "friendships_insert" on public.friendships;
create policy "friendships_insert" on public.friendships for insert with check (auth.uid() = sender_id);
drop policy if exists "friendships_update" on public.friendships;
create policy "friendships_update" on public.friendships for update using (auth.uid() = sender_id or auth.uid() = receiver_id or receiver_email = auth.jwt()->>'email');
drop policy if exists "friendships_delete" on public.friendships;
create policy "friendships_delete" on public.friendships for delete using (auth.uid() = sender_id or auth.uid() = receiver_id);

-- RLS for Squads
alter table public.squads enable row level security;
drop policy if exists "squads_select" on public.squads;
create policy "squads_select" on public.squads for select using (created_by = auth.uid() or public.is_squad_member(id, auth.uid()));
...
create policy "squads_update" on public.squads for update using (created_by = auth.uid());
drop policy if exists "squads_delete" on public.squads;
create policy "squads_delete" on public.squads for delete using (created_by = auth.uid());

-- RLS for Squad Members
alter table public.squad_members enable row level security;
drop policy if exists "squad_members_select" on public.squad_members;
create policy "squad_members_select" on public.squad_members for select using (user_id = auth.uid() or public.is_squad_member(squad_id, auth.uid()));
drop policy if exists "squad_members_insert" on public.squad_members;
create policy "squad_members_insert" on public.squad_members for insert with check (exists (select 1 from public.squads where id = squad_id and created_by = auth.uid()) or auth.uid() = user_id);
drop policy if exists "squad_members_update" on public.squad_members;
create policy "squad_members_update" on public.squad_members for update using (exists (select 1 from public.squads where id = squad_id and created_by = auth.uid()));
drop policy if exists "squad_members_delete" on public.squad_members;
create policy "squad_members_delete" on public.squad_members for delete using (user_id = auth.uid() or exists (select 1 from public.squads where id = squad_id and created_by = auth.uid()));

-- RLS for Event Participants
alter table public.event_participants enable row level security;
drop policy if exists "event_participants_select" on public.event_participants;
create policy "event_participants_select" on public.event_participants for select using (user_id = auth.uid() or public.is_event_creator(event_id, auth.uid()) or public.is_event_participant(event_id, auth.uid()));
drop policy if exists "event_participants_insert" on public.event_participants;
create policy "event_participants_insert" on public.event_participants for insert with check (public.is_event_creator(event_id, auth.uid()) or auth.uid() = user_id);
drop policy if exists "event_participants_update" on public.event_participants;
create policy "event_participants_update" on public.event_participants for update using (public.is_event_creator(event_id, auth.uid()));
drop policy if exists "event_participants_delete" on public.event_participants;
create policy "event_participants_delete" on public.event_participants for delete using (auth.uid() = user_id or public.is_event_creator(event_id, auth.uid()));
...
create policy "team_vault_assets_select" on public.team_vault_assets for select using ((squad_id is null and created_by = auth.uid()) or (squad_id is not null and public.is_squad_member(squad_id, auth.uid())));
...
create policy "team_vault_assets_insert" on public.team_vault_assets for insert with check (auth.uid() = created_by and (squad_id is null or public.is_squad_member(squad_id, auth.uid())));
...
create policy "team_vault_assets_update" on public.team_vault_assets for update using (auth.uid() = created_by);
...
create policy "team_vault_assets_delete" on public.team_vault_assets for delete using (auth.uid() = created_by);
